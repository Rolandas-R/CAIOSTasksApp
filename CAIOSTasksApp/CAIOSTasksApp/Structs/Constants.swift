...
enum Constants: String {
    ...
    static func buildGetUserTasksURL(userId: Int) -> URL? {
        
        if let url = getURL(for: .taskEndpoint, subEndpoint: .userTasks) {
            
            var components = URLComponents(url: url, resolvingAgainstBaseURL: true)
            let userIdQueryItem = URLQueryItem(name: "userId", value: String(userId))
